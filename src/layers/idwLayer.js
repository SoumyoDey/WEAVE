import { getDynamicColor } from '../utils/colorUtils';
 ...
 */
export const drawOnMap = (map, data, colormapName, isStdDev, range, refs) => {
  const { canvasRef, drawFnRef, uncertaintyModeRef } = refs;

  if (!map || !map._loaded) {
    setTimeout(() => drawOnMap(map, data, colormapName, isStdDev, range, refs), 200);
    return;
  }
  ...
  const draw = () => {
    ...
    for (let py = 0; py < tempCanvas.height; py++) {
      for (let px = 0; px < tempCanvas.width; px++) {
        ...
        if (
          screenX < topLeft.x || screenX > bottomRight.x ||
        ...
        for (const point of spatialData) {
          ...
          if (distance < influenceRadius) {
            const weight = distance < 1 ? 1 : 1 / (distance ** 4);
            weightedSum += point.value * weight;
            totalWeight += weight;
          }
        }

        const interpolated = totalWeight > 0 ? weightedSum / totalWeight : 0;
        const color = getDynamicColor(Math.max(interpolated, 0), range, colormapName);
        ...
        if (rgba) {
          ...
          imageData.data[idx+3] = totalWeight > 0
            ? (rgba[4] ? parseFloat(rgba[4]) * 255 : 255)
            : 100;
        }
      }
    }
