...
3. ~~**The `observation_data` ingest — the largest real piece of work left.**~~
   ...
   **What it created is larger than what it closed, and is now the top item on
   ...
   `METRICS_AUDIT.md` report is misaligned in time. Nothing is fixed yet; the
   size of the error has deliberately not been guessed at.
...
lead H against truth from H+4.** Two consequences follow structurally:

- **The precipitation record really reaches +23.5 h**, not the +19.5 h
  `/api/observation-coverage` reports (§4). Part of the wind/precipitation
  asymmetry that §4 explains by window coverage is this instead.
- **Every precipitation figure in `METRICS_AUDIT.md` is misaligned in time** —
  MAE, RMSE, bias, CSI, POD, FAR, FSS, CRPS, Brier. Wind's are fine.

**How much each score moves has not been measured, and no number here should be
guessed at.** That needs the truth field rebuilt at correct UTC and the
endpoints re-run — the same shape of job as the 2026-09-04 swap (§7), which is
the model to follow: measure first with `--compare`, keep the old table in
place, swap in one transaction. It is a decision about the live table, so it is
