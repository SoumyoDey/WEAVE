...
3. **The `observation_data` ingest — the largest real piece of work left**, and
   ...
   observations were ingested off-repo. `regrid_observations.py` handles
   everything above it (§7), and `DEPLOY.md` §2b says plainly that a fresh
   install gets forecasts and no truth until this exists. It is also what blocks
   `DATA_EXPANSION_DESIGN.md` phase 4 and therefore a second run.

   **No longer blocked on finding the data — 2026-09-16.** This item used to say
   the source files "are not on the development machine", which was true and
   was read for three weeks as though it meant they were nowhere. Both are on
   Explorer, with exact paths, contents and verification in §11. What is left is
   a loader, plus one decision about IMERG's temporal resolution that §11 states
   and does not make.
...
### IMERG precipitation — the run date is covered, but only at 6-hourly
...
**The decision this carries, which is not mine to make.** The file is *already
6-hourly accumulated* ("Accumulated from 30-minute instantaneous rates"), and
the truth field in the database today is **half-hourly** — that is what
`record_end_lead_hours = 19.5` means (§4), and what defect 2 was about, the
earliest window short by one of its **twelve** half-hourly samples (§3). A
6-hourly source cannot reproduce either number. Two options:

- **Accept 6-hourly.** Precipitation verification locks to the 00/06/12/18 grid.
  That aligns with `COMMON_VERIFICATION_WINDOW_HOURS`, so the primary scored
  path is unaffected. What it costs is everything at native cadence: UKMO's
  hourly records, the `(−1, 0]` window at +0 h (§3b), and the half-hour
  precision in the coverage endpoint.
- **Re-download half-hourly granules** for the run date from GES DISC. The
  download notebook is in that same directory
  (`IMERG/.ipynb_checkpoints/IMERG_DATA_Download-checkpoint.ipynb`).
