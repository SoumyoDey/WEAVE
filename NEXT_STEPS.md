...
- **2026-09-16 — `observation_data` has a loader, and it found a defect in the
  truth field** (§12). `Data/load_observations.py` reproduces **all 2,480,664
  rows** of the loaded run bit-for-bit, which closes the last hole in the
  install path. Getting it to reproduce them is what exposed the defect:
  **IMERG's stored timestamps are 4 hours behind UTC and ERA5's are not**, so
  every precipitation score pairs a forecast at lead H against truth from
  H+4. Wind is unaffected. Nothing has been fixed yet — read §12 before
  trusting any precipitation figure, including `METRICS_AUDIT.md`'s.
...
3. ~~**The `observation_data` ingest — the largest real piece of work left.**~~
   ...
   **What it created is larger than what it closed, and is now the top item on
   ...
   `METRICS_AUDIT.md` report is misaligned in time. **Measured 2026-09-16**
   (§12): continuous errors fall 4–11% domain-wide and up to 67% at a point,
   UKMO's precipitation bias flips sign, and FSS/CSI move in *opposite
   directions by model* — so the shift was changing which model looks better,
   not just inflating everyone's error. Wind is byte-identical, the control
   that confirms it is IMERG's alone. **Nothing is fixed yet**; §12 has the
   sequence, and the corrected field is already staged in the `utc` schema.
...
### Fixing it — not done, and the shape it should take

Deliberately not applied: it is a decision about the live table. The 2026-09-04
swap (§7) is the model to follow, and the staged corrected field already exists
in the `utc` schema.

1. `UPDATE observation_data SET obs_time = obs_time + interval '4 hours' WHERE source = 'GPM_IMERG_V07B'` — the raw table is the root, and leaving it shifted means the next regrid reintroduces the defect.
2. Rebuild `regridded_observation` from it, or apply the same relabel, in **one
   transaction** so there is never a moment with no truth field.
3. Clear `.cache/plots` and **restart every server** — the standing trap in §1.
4. Re-derive `METRICS_AUDIT.md`, and correct §4's `record_end_lead_hours` of
   19.5 to 23.5 wherever it is quoted.
5. Keep the old table in place (`regridded_observation_shifted`) as §7 did,
   rather than relying on a dump.

`load_observations.py` already defaults to UTC, so **a future load is correct
whether or not this is fixed** — which also means loading a second run before
fixing this would leave two runs on two different time bases in one table.
