...
>
> **§0 below carries the re-derived figures**, generated by
> `Data/rederive_audit.py` with every parameter printed beside its number — which
> the original figures did not have, and which is why several of them cannot be
> recomputed at all rather than merely being out of date. Individual stale
> numbers in the body are marked where they appear.
>
> The reasoning, the retracted findings, and the unit/window conventions all
> stand. It is the arithmetic that moved, and in two places the re-derivation
> made a finding *sharper* rather than weaker — see §0 on finding 8.
...
*/api/compare/skill at 36.0 N, 75.5 W (the coastal point used throughout the audit), hours 0-168*
...
*/api/compare/region-metrics over 25-45 N, 85-65 W (the whole analysis grid), hours 0-168*
...
*/api/spatial-metric metric=mae over 25-45 N, 85-65 W (the whole analysis grid)*
...
*25-45 N, 85-65 W (the whole analysis grid), every stored record*
...
- Region means match `/api/spatial-metric` exactly (MAE 0.5720 / 54 cells;
  CSI 0.2901 / 27 cells). **[pre-2026-09-04 truth field — see §0]** The *agreement* is what this line
  asserts and it still holds — §0 re-derives both sides from the same endpoints.
  The box that produced 54 cells was never recorded, so the values cannot be
  recomputed as-is.
...
- **Regression guard:** UKMO precipitation and all wind numbers were
  byte-for-byte unchanged (UKMO bias 0.106, mae 0.874, csi 0.304, ssr_agg 4.038;
  AIFS wind bias 1.579, mae 3.849, ssr_agg 0.978) — exactly right at the time,
  since neither model's semantics changed. **[pre-2026-09-04 truth field — see §0]** These are no longer a
  usable guard: the truth field itself changed on 2026-09-04, so *every* score
  moved and byte-equality is the wrong expectation. §0 has the current values.
...
(Jensen), and pooled CSI `0.208` against `0.157`. **[pre-2026-09-04 truth field — see §0]**
Re-derived over the full domain: pooled RMSE `1.0171` against `0.4621`, a ratio
of 2.20; pooled CSI at 6 mm/6h `0.2621` against `0.195`. The *finding* is
unchanged and in fact sharper — and note MAE now shows a wrinkle the original
missed: it is linear, so the two estimators agree exactly for AIFS and UKMO but
not for GEFS, whose cells do not all contribute the same number of samples. `correlation` has no pooled
...
observed                17.64 mm over 48 h   (0.368 mm/h x 48)   <- stale, §0
```
...
```
  ...
  observed                           0.387 mm/h   <- stale; 0.3999 now, §0
...
Domain mean 0.158 mm/h against 0.387 observed and 0.418 for UKMO. **[pre-2026-09-04 truth field — §0]** Region suite
